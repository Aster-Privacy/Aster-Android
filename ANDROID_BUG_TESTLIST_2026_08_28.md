...
| D26 | Mail list | A deleted message sometimes flickered back into the list before disappearing again, because the optimistic removal guard was released as soon as the request returned and a page fetch already in flight re-added the item | A short lived removal tombstone keyed by folder and item id outlives the request, is consulted at both merge filter points, and is cleared on undo and on restore | Emulator: delete stays gone across 12 seconds of refreshes, undo still restores, and restoring out of Trash still shows the message in Inbox |
