...
class RatchetStateSyncer @Inject constructor(
    ...
    suspend fun fetch_from_server(conversation_id: String): RatchetState? {
        ...
        return try {
            ...
            android.util.Log.w("AsterRatchet", "fetch_from_server failed", t)
            null
        } finally {
            key.fill(0)
        }
    ...
    suspend fun reset(conversation_id: String) {
        lock_for(conversation_id).withLock {
            ...
            try {
                ...
                android.util.Log.w("AsterRatchet", "reset: server delete failed (continuing)", t)
            }
        }
    }
    ...
    private suspend fun do_sync(conversation_id: String, state: RatchetState): Boolean {
        ...
        try {
            ...
            for (attempt in 0 until MAX_ATTEMPTS) {
                ...
                when (val outcome = try_put(conv_b64, ct_b64, nonce_b64, current_version)) {
                    ...
                    PutStateOutcome.NotFound -> {
                        android.util.Log.w("AsterRatchet", "sync put 404 -> wiping local state to force rebootstrap on next send")
                        known_versions.remove(conversation_id)
                        state_store.delete(conversation_id)
                        return false
                    }
            ...
            android.util.Log.w("AsterRatchet", "sync exhausted attempts ($last_error) -> wiping local state to force rebootstrap")
            known_versions.remove(conversation_id)
            state_store.delete(conversation_id)
            return false
        } catch (t: Throwable) {
            android.util.Log.w("AsterRatchet", "sync threw", t)
            return false
        } finally {
            key.fill(0)
        }
