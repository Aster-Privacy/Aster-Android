...
class ServerCodeErrorTest {
    ...
    fun carries_the_server_code_on_forbidden_and_conflict() {
        assertEquals("ACCOUNT_SUSPENDED", ApiError.ForbiddenError("nope", "ACCOUNT_SUSPENDED").server_code)
        assertEquals("CLIENT_UPGRADE_REQUIRED", ApiError.Conflict("nope", "CLIENT_UPGRADE_REQUIRED").server_code)
        assertNull(ApiError.ForbiddenError("nope").server_code)
        assertNull(ApiError.Conflict("nope").server_code)
    }
}
